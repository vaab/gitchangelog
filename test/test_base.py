...
class GitChangelogTest(BaseGitReposTest):
    ...
    def setUp(self):
        ...
        self.git.commit(
            message="new: first commit", author="Bob <bob@example.com>", date="2000-01-01 10:00:00", allow_empty=True
        )
        self.git.tag("0.0.1")
        self.git.commit(
    ...
    def test_simple_run_no_args_legacy_call(self):
        ...
        self.assertEqual(err, "", msg="There should be no standard error outputed. " "Current stderr:\n%r" % err)
        self.assertEqual(errlvl, 0, msg="Should not fail on simple repo and without config file")
        self.assertContains(
            out, "0.0.2", msg="At least one of the tags should be displayed in stdout... " "Current stdout:\n%s" % out
        )
    ...
    def test_simple_run_show_call_deprecated(self):
        ...
        self.assertContains(
            err, "deprecated", msg="There should be a warning about deprecated calls. " "Current stderr:\n%r" % err
        )
        self.assertEqual(errlvl, 0, msg="Should not fail on simple repo and without config file")
        self.assertContains(
            out, "0.0.2", msg="At least one of the tags should be displayed in stdout... " "Current stdout:\n%s" % out
    ...
    def test_incremental_call(self):
        ...
        self.assertEqual(errlvl, 0, msg="Should not fail on simple repo and without config file")
        self.assertContains(
            out, "0.0.3", msg="The tag 0.0.3 should be displayed in stdout... " "Current stdout:\n%s" % out
        )
    ...
    def test_incremental_show_call_deprecated(self):
        ...
        self.assertEqual(errlvl, 0, msg="Should not fail on simple repo and without config file")
        self.assertContains(err, "deprecated", msg="There should be a deprecated warning. " "Current stderr:\n%r" % err)
        self.assertContains(
            out, "0.0.3", msg="The tag 0.0.3 should be displayed in stdout... " "Current stdout:\n%s" % out
        )
        self.assertNoDiff(self.INCR_REFERENCE_002_003, out)
    ...
    def test_provided_config_file(self):
        ...
        config_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "..", "src", "gitchangelog")
        configs = glob.glob(os.path.join(config_dir, "gitchangelog.rc.reference.v*"))
        self.assertNotEqual(len(configs), 0)
        for config in configs:
            out, err, errlvl = cmd("$tprog", env={"GITCHANGELOG_CONFIG_FILENAME": config})
    ...
    def test_same_output_with_different_engine(self):
        ...
        gitchangelog.file_put_contents(".gitchangelog.rc", "output_engine = mustache('restructuredtext')")
        changelog = w("$tprog")
        self.assertNoDiff(self.REFERENCE, changelog)

        gitchangelog.file_put_contents(".gitchangelog.rc", "output_engine = makotemplate('restructuredtext')")
        changelog = w("$tprog")
        self.assertNoDiff(self.REFERENCE, changelog)
    ...
    def test_provided_templates(self):
        ...
        for label, directory in [("makotemplate", "mako"), ("mustache", "mustache")]:
            template_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "..", "templates", directory)
            templates = glob.glob(os.path.join(template_dir, "*.tpl"))
            template_labels = [os.path.basename(f).split(".")[0] for f in templates]
            for tpl in template_labels:
                gitchangelog.file_put_contents(".gitchangelog.rc", "output_engine = %s(%r)" % (label, tpl))
                out, err, errlvl = cmd("$tprog")
                self.assertEqual(
                    errlvl, 0, msg="Should not fail on %s(%r) " % (label, tpl) + "Current stderr:\n%s" % indent(err)
                )
    ...
    def test_run_packages_not_in_config(self):
        ...
        self.assertContains(
            err.lower(),
            "missing value",
            msg="There should be an error message containing 'missing value'. " "Current stderr:\n{err}",
        )
        self.assertContains(
            err.lower(),
            "config file",
            msg="There should be an error message containing 'config file'. " "Current stderr:\n{err}",
        )
        self.assertEqual(errlvl, 1, msg="Should have failed.")
        self.assertEqual(out, "", msg="No output is expected.")

    def test_run_package_arg_missing_in_config(self):
        gitchangelog.file_put_contents(".gitchangelog.rc", "packages = {}")
        package = "app"
        out, err, errlvl = cmd(f"$tprog --package {package}")
        self.assertContains(
            err.lower(),
            f"package '{package}'",
            msg=f"There should be an error message containing 'Package '{package}''. " f"Current stderr:\n{err}",
        )
        self.assertContains(
            err.lower(),
            "not defined in config file",
            msg="There should be an error message containing 'not defined in config file'. " "Current stderr:\n{err}",
        )
        self.assertEqual(errlvl, 1, msg="Should have failed.")
        self.assertEqual(out, "", msg="No output is expected.")
