...
class GitReposInstantiationTest(BaseTmpDirTest):
    ...
    def test_gitchangelog_on_non_git_repos_dir(self):
        ...
        self.assertEqual(err.strip(), "Not in a git repository. (calling ``git remote`` failed.)")
...
class GitReposTest(BaseGitReposTest):
    ...
    def test_get_commit(self):
        ...
        self.assertEqual(commit.subject, "add ``b`` with non-ascii chars éèàâ§µ and HTML chars ``&<``")
